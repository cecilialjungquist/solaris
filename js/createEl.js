...
function createHeaderEl(planet) {
    ...
    let star = createStarEl(planet.latinName);
...
function createStarEl(latinName) {
    ...
    if (checkStarredPlanet(latinName) > -1) {
        classCSS = 'svg--star-fill';
        isStarred = 'true';
    }

    star.innerHTML = `
    <svg class="${classCSS}" data-starred-planet="${latinName}" data-is-starred="${isStarred}" width="45" height="42" viewBox="0 0 182 163" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    star.addEventListener('click', () => starToggle(star));
    return star;  
}
